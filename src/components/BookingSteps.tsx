    ...
    <section id="steps" className="py-20 bg-slate-50 overflow-hidden border-t border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-1/2 left-[15%] right-[15%] h-0.5 bg-gradient-to-r from-amber-400 via-amber-500 to-teal-500 -translate-y-12 z-0 opacity-40" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
            {stepsList.map((stepItem, index) => (
              <motion.div
                ...
                key={index}
                className="bg-white rounded-3xl p-8 shadow-md border border-slate-200/90 hover:shadow-xl hover:border-amber-400 transition-all duration-300 flex flex-col items-center text-center relative group"
              >
                {/* Step Number Circle */}
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-500 to-teal-600 text-white flex items-center justify-center font-display font-black text-xl shadow-lg shadow-amber-500/20 mb-6 group-hover:scale-110 transition-transform duration-300">
                  {stepItem.step}
                </div>
                
                <h3 className="font-display font-black text-xl text-[#0f172a] mb-3 group-hover:text-amber-600 transition-colors uppercase tracking-tight">
                  {stepItem.title}
                </h3>
